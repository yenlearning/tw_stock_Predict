...
#==============================================================================#
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic', stockNo=None):
        plt.cla()
        self.reset_plot()
        ...
        for i, stat in enumerate(stats):
            ...
            if plot_type == 'pct':
                ...
                # plt.xlabel('Date'); 
                plt.ylabel('Change Relative to Average (%)'); plt.title('%s Stock History' % self.symbol); 
            ...
            # Stat y-axis
                ...
                # plt.xlabel('Date'); 
                plt.ylabel('Price'); plt.title('%s Stock History' % self.symbol); 
                plt.legend(prop={'size':10})
                plt.grid(color = 'k', alpha = 0.4); 
        ...
        plt.savefig('../static/img/img_history/'+'history_'+stockNo+'.png')
        # plt.show()
    ...
    def create_prophet_model(self, days=0, resample=False, stockNo='0000'):
        plt.cla()
        self.reset_plot()
        ...
        draw_future['ds']=future['ds'][-11:]
        draw_future['yhat']=future['yhat'][-11:]
        draw_future['yhat'].iloc[0]=stock_history['y'].iloc[-1].copy()

        # ax.plot(future['ds'], future['yhat'], 'forestgreen',linewidth = 2.4, label = 'Modeled')
        ax.plot(draw_future['ds'], draw_future['yhat'], 'forestgreen',linewidth = 2.4, label = 'Modeled')
        #找明天的時間
        tomorrow=datetime.strptime(str(future['ds'].iloc[-1]), "%Y-%m-%d %H:%M:%S")
        print(tomorrow)
        ...
        plt.xlim(xmin = date(2021, 1, 1),xmax=tomorrow)
        
        plt.ylim([stock_history['y'][-150:].min()-20   ,  stock_history['y'][-150:].max()+20])
        
        plt.grid(linewidth=0.6, alpha = 0.6)
        plt.title(title)
        plt.savefig('../static/img/img_prophet/'+'prophet_'+stockNo+'.png')
        # plt.show()
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        # change the default date to 3 months
        # Default end date is end date of data
        if start_date is None:
            start_date = self.max_date - pd.DateOffset(years=0.25)
        if end_date is None:
            end_date = self.max_date
